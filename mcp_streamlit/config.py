...
def _default_output_root() -> str:
    ...
    candidates = [
        here.parent / "output_files",                                # mcp_ai/output_files
        here.parent.parent / "mcp_ai" / "output_files",              # repo-root/mcp_ai/output_files
...
class Settings(BaseModel):
    ...
    CLOUD_LLM_PROVIDER: str = "none"  # "none" | "azure-openai" | "openai"
    ...
    # Optional: command Streamlit can run from the Scan page
    SCAN_COMMAND: Optional[str] = Field(default=os.getenv("SCAN_COMMAND"))

    # Where to read YAML product config (your existing Config.yaml)
    ...
    def load_yaml_overrides(self) -> None:
        ...
        # Local (ollama) compatibility bridge
        ...
        if prov in ("none", "azure-openai", "openai"):
